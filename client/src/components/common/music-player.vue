<template>
  <div class="player-container">
    <div :class="containerClass" ref="container">
      <audio :src="music.src" preload="auto" ref="audio" @ended="handleAudioEnd" @canplaythrough="handleCanPlayThrough($event)" @durationchange="handleDurationChange($event)" @timeupdate="handleTimeUpdate"></audio>
      <div class="uplayer-info">
        <div class="uplayer-info-cover"><img :src="music.coverSrc || 'https://picsum.photos/100/?random'" alt="cd-cover"></div>
        <div class="uplayer-meta">
          ...
          <div class="uplayer-meta-time-tick">
            <span class="uplayer-meta-time-tick-text" ref="timeTick"></span>
          </div>
        </div>
      </div>
      <canvas class="uplayer-spectrum" ref="canvas"></canvas>
      <div class="uplayer-lyric" ref="lyricArea">
        <div class="uplayer-lyric-area"></div>
      </div>
      <div class="uplayer-control">
        <div class="uplayer-control-play" ref="playBtn" @click="handlePlayClick">
          <i class="icon-music-play"></i>
          <i class="icon-music-pause"></i>
        </div>
      </div>
      <div class="uplayer-volume-bg">
        <div class="uplayer-volume" ref="volumeArea">
          <i class="icon-music-volume"></i>
          <div class="uplayer-volume-progress" ref="volumeProgress"></div>
        </div>
      </div>
      <div class="uplayer-duration" ref="duration">
        <i class="icon-music-clock"></i>
        <span class="uplayer-duration-text">{{durationText}}</span>
      </div>
      <div class="uplayer-loadingsign">
        <i class="icon-music-spin animate-spin"></i>loading</div>
      <div class="uplayer-timeline-bg">
        <div class="uplayer-timeline" ref="timeline" @click="handleTimeLineClick($event)">
          <div class="uplayer-timeline-passed" ref="timePassed"></div>
        </div>
      </div>
    </div>
  </div>
