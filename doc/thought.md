...
10. 本地因为直接访问的`localhost:3333`端口，所有请求都会到这里，但是线上是80端口通过nginx把`/`请求转到了前台3000端口，`/api`请求转到了后端8000端口，所以跨域的请求也被直接转到了后端，就没有走nuxt配置的proxy，所以还需要在nginx里对`/api/proxy`这种跨域请求做处理，将其转到3000端口上，其实也可以直接把proxy放到后端，这样就没有这个问题了。
